...
data_non_malaria = readRDS(paste0(data_path, "non_malarial_infections.RDS")) %>%
  dplyr::select(
    id,
    febrile.illness,
    non.malarial.febrile,
    respiratory.diagnosed,
    genitourinary.diagnosed,
    gastrointestinal.diagnosed,
    integumentary.diagnosed,
    abdpain.diagnosed,
    headache.diagnosed ,
    other.pain.diagnosed
  )

data_medication = readRDS(paste0(data_path, "maternal_medication.RDS")) %>%
  dplyr::select(
    motherid,
    anymed,
    antibacterial,
...
# ! med and infection to be adjusted in the future!

maternal_med_covariate_list = c("antibacterial", "betalactam", "fluoroquinolone", "sulfonamide", "antibacterial_other", "antimalarial",
                                "antiparasitic", "antiviral", "anymed")
...
main_covariate_list = c("sex", "enrollage", "maternal_agecat", "GAenroll", "Gravidity", "gravidity_cat", "LBW", "anyHP", "preterm", "birthweight", "birthweight_kg", 
                        ...
                        "febrile.illness_binary", "non.malarial.febrile_binary", "respiratory.diagnosed_binary",
                        "genitourinary.diagnosed_binary", "gastrointestinal.diagnosed_binary", "integumentary.diagnosed_binary",
                        "abdpain.diagnosed_binary", "headache.diagnosed_binary", "other.pain.diagnosed_binary",
                        "any_nonmalaria_illness_binary", "anymed_binary", "antibacterial_binary", "betalactam_binary",
                        "fluoroquinolone_binary", "sulfonamide_binary", "antibacterial_other_binary", "antimalarial_binary",              
                        "antiparasitic_binary", "antiviral_binary", "any_nonmalaria_med_binary") 
...
# non-malaria illness
data_non_malaria[is.na(data_non_malaria)] <- 0
data_non_malaria_binary = data_non_malaria %>%
  group_by(id) %>%
  dplyr::summarize(across(everything(), ~ ifelse(sum(.) >= 1, 1, 0), .names = "{.col}_binary")) %>%
  ungroup() %>%
  mutate(any_nonmalaria_illness_binary = ifelse(rowSums(dplyr::select(., -c(id, febrile.illness_binary))) == 0, 0, 1)) %>%
  mutate(across(-id, as.factor))

maternal_main = maternal_main %>%
  left_join(data_non_malaria_binary, by = c("motherid" = "id"))


# medication
data_med_binary = data_medication %>%
  group_by(motherid) %>%
  dplyr::summarize(across(everything(), ~ ifelse(sum(.) >= 1, 1, 0), .names = "{.col}_binary")) %>%
  ungroup() %>%
  mutate(any_nonmalaria_med_binary = ifelse(rowSums(dplyr::select(., -c(motherid, antimalarial_binary))) == 0, 0, 1)) %>%
  mutate(across(-motherid, as.factor)) 

maternal_main = maternal_main %>%
  left_join(data_med_binary, by = c("motherid" = "motherid"))
...
#View(data)
# !no variability in antiviral_binary
...
# -----------------------------------------
# continuous
# (GEE takes care of repeated monthly measurements)
# -----------------------------------------
